...
/******** OWNER NUMBER**********/
const ownerNumber = ["6285894628645@s.whatsapp.net","6281289826498@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('Rixal','blue'))
})
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
                ...
                await reply('Mencari partner...')
                await reply(`wa.me/${anug}`)
                await reply( `Partner ditemukan: 🙉\n*${prefix}next* — temukan partner baru`)
            break
            case 'next':
                if (!isRegistered) return reply( ind.noregis())
                if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
...
case 'loli':
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				    try {
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/waifu?apikey=BotWeA`)
						buffer = await getBuffer(res.image)
               ...
               if (data.error) return reply(data.error)
                ...
                case 'quotes':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/randomquotes?apikey=BotWeA`, {method: 'get'})
					reply(anu.quotes)
					await limitAdd(sender)
					break		
					case 'infonomor':
                ...
                case 'beritahoax':
					...
					reply('Makasih profil barunya😗 Darling')
					break 
					case 'brainly':
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
